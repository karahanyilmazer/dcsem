...
def new_objective(param, bold_signal, est_name, all_params):
    for p, v in zip(est_name, param):
        all_params[p] = v
    bold_simulated = simulate_bold(all_params, time=time, u=u, num_rois=num_rois,)
    return np.mean((bold_simulated - bold_signal) ** 2)


opt = minimize(new_objective, x0=initial_values,
               args=(bold_noisy, params_to_est, true_params.copy()),
               bounds=bounds, method='L-BFGS-B',)

estimated_params = dict(zip(params_to_est, opt.x.tolist()))
