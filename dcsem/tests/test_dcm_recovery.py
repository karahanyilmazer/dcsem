...
def test_3roi_dcm_forward_simulation_and_recovery_smoke():
    """3-ROI clean-BOLD smoke: simulate a chain edge and recover it."""
    dcm = models.DCM(3)
    A_true = np.array(
        [
            [-1.5, 0.0, 0.0],
    ...
    C_true = np.array([0.8, 0.0, 0.0])
    dcm.set_params({"A": A_true, "C": C_true})

    tvec = np.linspace(0, 30, 90)
    u = utils.stim_boxcar([[0, 5, 1]])
    y, _ = dcm.simulate(tvec, u=u)

    assert y.shape == (len(tvec), 3)
    ...
    assert res.success
    assert np.isclose(res.x[names.index("a1_0")], A_true[1, 0], atol=7e-2)
    assert np.isclose(res.x[names.index("c0")], C_true[0], atol=7e-2)
