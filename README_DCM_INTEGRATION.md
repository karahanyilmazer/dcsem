...
Both `inversion_generic.py` and `mcmc_generic.py` now support DCM 2-ROI BOLD models alongside analytical models.

## Quick Start

### For Optimizer-Based Estimation (`inversion_generic.py`)

1. Open `inversion_generic.py`
...
### For MCMC-Based Estimation (`mcmc_generic.py`)

1. Open `mcmc_generic.py`
2. Comment out the active model (e.g., quadratic)
...
### MCMC Settings (mcmc_generic.py only)
```python
...
### Optimizer-Based (`inversion_generic.py`)
```
