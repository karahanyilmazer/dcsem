...
class BenchPanel(ControlPanel):
    ...
    def __init__(self, parent, overlayList, displayCtx, viewPanel):
        ...
        self.probmaps = []
        self.amoutmaps = []
        self.loadButton = wx.Button(self, label='Load Results Directory', size=(150, 50))
        self.sizer = wx.BoxSizer(wx.VERTICAL)

        self.figure, self.axis = plt.subplots(nrows=2, ncols=1, sharex=True,
    ...
    def onLoad(self, event):
        ...
        self.probmaps = [Image(f'{benchdir}/{m}_probability.nii.gz') for m in self.param_names]
        amountmaps = [Image(f'{benchdir}/{m}_amount.nii.gz') for m in self.param_names[1:]]
        amountmaps.insert(0, amountmaps[0].data * 0)
        self.amoutmaps = amountmaps

        fslsettings.write('fsleyes.bench.dir', benchdir)
        self.overlayList.append(Image(f'{benchdir}/inferred_change.nii.gz'), overlayType='label')
    ...
    def locationChanged(self):
        ...
        for ax, l, v in zip(self.axis, ('Change Probability', 'Amount of Change'), (probvals, amountvals)):
