...
def plot_conf_mat(conf_mat, param_names, title=None):
    ...
    ax = sns.heatmap(
        conf_mat.T, annot=True, fmt="2.2f", vmin=0, vmax=1, annot_kws={"size": 8}
    )
...
def bar_plot(measures, names, bvals, summary_type="dt"):
    ...
    for j, (m, idx) in enumerate(zip(summary_names, [m1_idx, m2_idx])):
        ...
        for i, meas in enumerate(measures):
            plt.bar(
                x=bvals + bar_width * (i - n_meas / 2) + bar_width / 2,
                height=np.squeeze(meas)[idx],
                width=bar_width,
                label=names[i],
                color=colors[i],
            )
...
def arrow_plot(
    ...
    for b_i, b in enumerate(bvals):
        ...
        for i, ch in enumerate(changes):
            ...
            ax[b_i].scatter(
                ch[idx1] + base[idx1], ch[idx2] + base[idx2], color=colors[i]
            )
            ax[b_i].text(
                base[idx1] + ch[idx1] + offset,
                base[idx2] + ch[idx2] + offset,
                names[i + 1],
                ha="left",
                va="top",
                size=12,
...
def plot_gaussian_2d(mean, cov, ax, color, xlim=0.1, ylim=0.1):
    ...
    cs = ax.contourf(X, Y, Z, colors=color, alpha=0.5, levels=np.logspace(-3, 0, 4))
