...
class Trainer:
    ...
    amount_priors: Union[float, Sequence] = (
        1  # model priors (different from param/change priors)
    )
    training_done = False
    ...
    def __post_init__(self):
        ...
        for idx in range(self.n_vecs):
            for pname in self.change_vecs[idx].keys():
                if pname not in self.param_names:
                    raise KeyError(
                        f"parameter {pname} is not defined in the forward model."
        ...
        try:
            ...
            print(
                "The provided function does not work with the given parameters and args."
            )
            raise ex
        if self.n_dim == 1:
            raise ValueError(
                "The forward model must produce at least two dimensional output."
            )
    ...
    def param_names(
        ...
        return [
            i for p in self.priors.keys() for i in ([p] if isinstance(p, str) else p)
        ]
    ...
    def train(
        ...
        def func(idx):
            ...
            if verbose:
                print(
                    f" mu optimization for {self.vec_names[idx]}: {mu_weights.message}\n "
            ...
            sigma_weights = scipy.optimize.minimize(
                neg_log_likelihood,
                method="BFGS",
                x0=x0,
                args=(
            ...
            if verbose:
                print(
                    f" sigma optimization for {self.vec_names[idx]}: {sigma_weights.message}\n "
                    f"function value:{sigma_weights.fun}, {sigma_weights.nit}"
                )
            else:
                print(
                    f"optimized regression weights for {self.vec_names[idx]} covariance"
                )

            all_weights = scipy.optimize.minimize(
                neg_log_likelihood,
                method="BFGS",
            ...
            if verbose:
                print(
                    f" mu+sigma optimization for {self.vec_names[idx]}: {all_weights.message}\n "
                    f"function value:{all_weights.fun}, {all_weights.nit}"
                )
            else:
                print(
                    f"optimized regression weights for {self.vec_names[idx]} joint mean and covariance"
                )

            w_mu = all_weights.x[: (n_mu_features * self.n_dim)].reshape(
                n_mu_features, self.n_dim
            )
        ...
        models = run_parallel(
            func, len(self.change_vecs), debug=not parallel, prefer="threads"
        )
        ...
        return ChangeModel(
            ...
            baseline_kde=kde,
            forward_model=self.forward_model.__name__,
            normaliser=self.normaliser,
        )

    def generate_train_samples(
        self, n_samples: int, dv0: float = 1e-6, base_params=None
    ) -> tuple:
        """
    ...
    def generate_test_samples(
        ...
        models = []
        for vec, name, prior, lims in zip(
            self.change_vecs, self.vec_names, self.amount_priors, self.lims
        ):
            for l in lims:
                models.append(
                    MLChangeVector(
        ...
        def generator_func(s_idx):
            ...
            while not valid:
                ...
                if tc == 0:
                    ...
                    params_2 = {
                        k: v + models[tc - 1].vec.get(k, 0) * effect_size[s_idx] * sign
                        for k, v in params_1.items()
                    }

                    valid = np.all(
                        [
                            self.priors[k].pdf(params_2[k]) > 0
                            if k in self.priors and hasattr(self.priors[k], "pdf")
                            else 0 <= params_2[k] <= 1
                    ...
                    if not valid and base_params is not None:
                        raise ValueError(
                            "Cant simulate a change with the baseline "
            ...
            if has_noise_model:
                ...
                y_1 = (
                    self.forward_model(**kwargs, **params_1)
                    + np.random.randn(self.n_dim) * noise_std
                )
                y_2 = (
                    self.forward_model(**kwargs, **params_2)
                    + np.random.randn(self.n_dim) * noise_std
                )
...
class MLChangeVector:
    ...
    def distribution(self, y):
        ...
        mu, sigma_inv, _ = regression_model(
            yf_mu, yf_sigma, self.mu_weight, self.sig_weight, self.diag_idx
        )
    ...
    def log_lh_precalc(self, y, dy, sigma_n):
        ...
        det_term = -0.5 * np.log(np.linalg.det(sn_inv)) + len(mu) / 2 * np.log(
            np.pi * 2
        )
        ...
        def f_noinv(x):
            new_evals = 1 / (
                1 + x**2 * evals
            )  # should make robust to being zero or negative
            e_term = -0.5 * np.sum(
                yt * ytb * new_evals
...
class ChangeModel:
    ...
    def estimate_change_vectors(self, data):
        ...
        data_norm, _, _ = self.normaliser(
            baseline=data, change=None, noise_cov=None, names=self.measurement_names
        )
    ...
    def infer(self, data, delta_data, sigma_n, integral_bound=1, parallel=True):
        ...
        y, dy, sn = self.normaliser(
            baseline=data,
            change=delta_data,
            noise_cov=sigma_n,
            names=self.measurement_names,
        )
        lls, amounts = self.compute_log_likelihood(
            y, dy, sn, parallel=parallel, integral_bound=integral_bound
        )
        ...
        posteriors[posteriors.sum(axis=1) == 0, 0] = (
            1  # in case all integrals were zeros accept the null model.
        )
        ...
        posteriors[np.isposinf(posteriors)] = (
            np.nanmax(posteriors[np.isfinite(posteriors)]) + 1
        )
    ...
    def compute_log_likelihood(
        self, y, delta_y, sigma_n, integral_bound=1, parallel=True, gaussian_approx=True
    ):
        """
        ...
        def func(sam_idx):
            ...
            if np.isnan(y_s).any() or np.isnan(dy_s).any() or np.isnan(sigma_n_s).any():
                ...
                if np.linalg.matrix_rank(sigma_n_s) < sigma_n_s.shape[0]:
                    sigma_n_s += 0.01 * np.diag(
                        np.diag(sigma_n_s)
                    )  # regularises the noise covariance.

                log_prob[0] = self.models[0].log_posterior(0, y_s, dy_s, sigma_n_s)

                for vec_idx, ch_mdl in enumerate(self.models[1:], 1):
                    log_post_pdf = lambda dv: ch_mdl.log_posterior(
                        dv, y_s, dy_s, sigma_n_s
                    )
                    ...
                    if ch_mdl.lim == "positive":
                        ...
                        neg_peak, upper, lower = [
                            -np.exp(x)
                            for x in find_range(
                                lambda dv: log_post_pdf(-np.exp(dv)),
                        ...
                        if log_pdf_peak < -200 or neg_peak < 1e-6:
                            neg_int, neg_expected = 0.0, 0.0
                        else:
                            post_pdf = lambda dv: np.exp(
                                log_post_pdf(dv) - log_pdf_peak
                            )
                            neg_int = scipy.integrate.quad(
                                post_pdf,
                                lower,
                                upper,
                                points=[neg_peak],
                    ...
                    if ch_mdl.lim == "negative":
                        ...
                        if log_pdf_peak < -200 or pos_peak < 1e-6:
                            pos_int, pos_expected = 0.0, 0.0
                        else:
                            post_pdf = lambda dv: np.exp(
                                log_post_pdf(dv) - log_pdf_peak
                            )
                            pos_int = scipy.integrate.quad(
                                post_pdf,
                                lower,
                                upper,
                                points=[pos_peak],
                    ...
                    if ch_mdl.lim == "positive":
                        ...
                        amount[vec_idx] = (
                            pos_expected if pos_int > neg_int else neg_expected
                        )
        ...
        def new_func(sam_idx):
            ...
            if np.isnan(y_s).any() or np.isnan(dy_s).any() or np.isnan(sigma_n_s).any():
                ...
                for vec_idx, ch_mdl in enumerate(self.models[1:], 1):
                    ...
                    log_post_pdf_flipped = lambda dv: ch_mdl.log_posterior(
                        -dv, y_s, dy_s, sigma_n_s
                    )

                    if ch_mdl.lim == "positive":
                        integral, peak = calc_integral_fast(
                            log_post_pdf, integral_bound
                        )

                    elif ch_mdl.lim == "negative":
                        integral, peak = calc_integral_fast(
                            log_post_pdf_flipped, integral_bound
                        )
                        peak = -peak
                    else:  # two-sided
                        pos_integral, pos_peak = calc_integral_fast(
                            log_post_pdf, integral_bound
                        )
                        neg_integral, neg_peak = calc_integral_fast(
                            log_post_pdf_flipped, integral_bound
                        )
        ...
        if gaussian_approx:
            log_probs1, amounts1 = run_parallel(
                new_func, n_samples, debug=not parallel, prefer="threads"
            )
        else:
            log_probs1, amounts1 = run_parallel(
                func, n_samples, debug=not parallel, prefer="threads"
            )
...
def dict_to_string(dict_):
    str_ = " ".join([f"{v:+1.1f}*{p}" for p, v in dict_.items() if v != 0]).replace(
        "1.0*", ""
    )
    if str_[0] == "+":
        str_ = str_[1:]
        ...
        if len(sub) > 1:
            ...
            for l in this_lims:
                if l not in INTEGRAL_LIMITS:
                    raise ValueError(
                        f"limits must be one of {INTEGRAL_LIMITS} but got {l}."
                    )
        else:
            this_lims = ["twosided"]
...
def knn_estimation(y, dy, k=100, lam=1e-12):
    ...
    for vec_idx in range(n_vecs):
        for sample_idx in range(n_samples):
            ...
            mu[vec_idx, sample_idx] = np.average(
                pop, axis=0, weights=weights[sample_idx]
            )
            try:
                c = np.cov(pop.T, aweights=weights[sample_idx])
                tril[vec_idx, sample_idx] = np.linalg.cholesky(c + lam * np.eye(dim))[
                    np.tril_indices(dim)
                ]
            except Exception as ex:
                print(ex)
...
def log_mvnpdf(x, mean, cov):
    ...
    nc = -0.5 * (
        log2pi * d + np.linalg.slogdet(cov)[1]
    )  # we expect the covariance be PD
...
def find_range(f: Callable, bounds, scale=1e-3):
    ...
    if f_norm(lower) < 0:  # to check the function is not flat.
        try:
            lower = scipy.optimize.root_scalar(
                f_norm, bracket=[lower, peak], method="brentq"
            ).root
        except Exception as e:
            print(
                f"Error of type {e.__class__} occurred while finding the lower limit of integral."
                f"The lower bound is used instead"
            )

    if f_norm(upper) < 0:
        try:
            upper = scipy.optimize.root_scalar(
                f_norm, bracket=[peak, upper], method="brentq"
            ).root
        except Exception as e:
            print(
                f"Error of type {e.__class__} occurred, while finding higher limit of integral."
...
def performance_measures(posteriors, true_change, set_names):
    ...
    true_posteriors = np.nanmean(
        posteriors[np.arange(len(true_change_idx)), true_change_idx]
    )
...
def neg_log_likelihood(
    weights, dy, yf_mu, yf_sigma, w_mu, diag_idx, opt_param="mu", alpha=0.1
):
    """
    ...
    if opt_param == "mu":
        ...
        mu, sigma_inv, ldet = regression_model(
            yf_mu, yf_sigma, w_mu_, w_sigma, diag_idx
        )
        offset = dy - mu
        nll = np.mean(
            -ldet + np.einsum("ij,ijk,ik->i", offset, sigma_inv, offset)
        ...
        w_sigma = weights[n_mu_features * n_dim :].reshape(
            n_sig_features, n_dim * (n_dim + 1) // 2
        )
...
def estimate_observation_likelihood(
    forward_model, kwargs, param_priors, n_samples=1000
):
    """
    :param forward_model:
    :param kwargs:
...
def run_parallel(func, n_iters, debug=False, print_progress=True, prefer=None):
    ...
    if debug is False:
        ...
        if print_progress is True:
            iters = tqdm.tqdm(
                iters, file=sys.stdout, total=n_iters, desc="Processing Jobs"
            )
    ...
    if isinstance(
        res[0], (np.ndarray, int, float)
    ):  # if output is a number return np.array
        return np.stack(res, axis=0)

    elif hasattr(
        res[0], "__len__"
    ):  # if output is multiple numbers return arrays for each if possible
        out = []
...
def summary_decorator(model, bvals, bvecs, summary_type="sh", sh_degree=2):
    ...
    if summary_type == "sh":
        ...
        names = [
            f"{b:1.3f}-{g[0]:1.3f},{g[1]:1.3f},{g[2]:1.3f}"
            for b, g in zip(bvals, bvecs)
        ]
    else:
        raise ValueError(
            f"Summary measurement type {summary_type} is not defined. Must be sh, dt or sig."
        )
...
def fit_transform(X, degree=1):
    ...
    if degree == 2:
        ...
        quadratic_terms = np.array(
            [X[:, i] * X[:, j] for i in range(K) for j in range(i, K)]
        ).T
...
def calc_integral_quad(func, max_x, eps_rel=1e-2):
    ...
    log_peak = scipy.optimize.minimize_scalar(
        neg_func_log, bounds=[-10, np.log(max_x)], method="bounded"
    ).x
    ...
    if norm_func(np.log(max_x)) > 0:
        ...
        log_upper = scipy.optimize.root_scalar(
            norm_func, bracket=[log_peak, np.log(max_x)], method="brentq"
        ).root
...
def calc_integral_fast(func, max_x, eps_rel=1e-3):
    ...
    f_peak = func(peak)
    if (
        peak < 1e-6 or f_peak < -200
    ):  # if the peak is too close to zero, or it's too short don't bother
        return 0.0, 0.0
