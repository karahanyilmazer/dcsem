...
theta_true = np.array(
    [2.0, 3.0, 5.0]
)  # [a, b, c] -> effective slope = 6.0, intercept = 5.0
theta_zero = np.array([1.0, 1.0, 0.0])  # Nonzero for a, b to avoid degeneracy
...
def hessian_analytical(theta, x, y):
    """Analytical Hessian of MSE for product model: y = a*b*x + c"""
    a, b, c = theta

    # Residuals
    r = a * b * x + c - y

    # Analytical Hessian for MSE
    H = np.zeros((3, 3))
    H[0, 0] = 2 * np.mean((b * x) ** 2)  # ∂²/∂a²
    H[1, 1] = 2 * np.mean((a * x) ** 2)  # ∂²/∂b²
    H[2, 2] = 2.0  # ∂²/∂c²
    H[0, 1] = H[1, 0] = 2 * np.mean(
        x**2 * (a * b + r)
    )  # ∂²/∂a∂b (includes residual term)
    H[0, 2] = H[2, 0] = 2 * np.mean(b * x)  # ∂²/∂a∂c
    H[1, 2] = H[2, 1] = 2 * np.mean(a * x)  # ∂²/∂b∂c
...
print(
    f"  Effective slope (a*b): true={theta_true[0]*theta_true[1]:.4f}, est={a_est*b_est:.4f}"
)
...
if PLOT_1D:
    ...
    span_b = SPAN_B if SPAN_B is not None else 1.5 * max(1.0, abs(b_est))
    span_c = SPAN_C if SPAN_C is not None else 0.7 * max(5.0, abs(c_est))
...
if PLOT_2D:
    ...
    # (a, b) with c fixed - CORRECTED for product model
    AA, BB = np.meshgrid(a_grid, b_grid, indexing="ij")
    CC = c_est
    Zab = np.mean((AA[..., None] * BB[..., None] * X + CC - YY) ** 2, axis=-1)

    # (a, c) with b fixed - CORRECTED
    AA2, CC2 = np.meshgrid(a_grid, c_grid, indexing="ij")
    BB2 = b_est
    Zac = np.mean((AA2[..., None] * BB2 * X + CC2[..., None] - YY) ** 2, axis=-1)

    # (b, c) with a fixed - CORRECTED
    BB3, CC3 = np.meshgrid(b_grid, c_grid, indexing="ij")
    AA3 = a_est
    Zbc = np.mean((AA3 * BB3[..., None] * X + CC3[..., None] - YY) ** 2, axis=-1)
...
if PLOT_3D:
    ...
    b_grid = make_range(b_est, 1.5 * max(1.0, abs(b_est)), N_2D)
    AA, BB = np.meshgrid(a_grid, b_grid, indexing="ij")
    X = x_data[None, None, :]
    YY = y_data[None, None, :]
    CC = c_est
    # CORRECTED for product model
    Zab = np.mean((AA[..., None] * BB[..., None] * X + CC - YY) ** 2, axis=-1)
...
if SHOW_DIAGNOSTICS:
    ...
    H_analytical = hessian_analytical(
        theta_est, x_data, y_data
    )  # CORRECTED: pass theta, x, y
    ...
    if rank_deficient:
        print(
            "⚠️ Warning: Hessian is rank-deficient or nearly singular. Skipping inversion."
        )
        cov = None
        corr = None
        se = np.array([np.nan, np.nan, np.nan])  # CORRECTED: 3 NaNs
        ...
        try:
            ...
            # Check for negative variances (numerical issue)
            ...
            se = np.sqrt(diag_cov)  # Standard errors
    ...
    # Warnings for poor conditioning
    if cond > 1e6:
    ...
    if np.isfinite(max_offdiag_corr):
        ...
        if max_offdiag_corr > 0.95:
            print(
                f"  ⚠️ High parameter correlation ({max_offdiag_corr:.3f}) - identifiability issues!"
            )
            print(
                f"     Note: For y=a*b*x+c, only the product a*b is identifiable, not a and b separately"
            )

    if not rank_deficient:
        print("  95% CI rows [low, high] per parameter (a,b,c):")  # CORRECTED label
        for i, (lo, hi) in enumerate(ci):
            print(
                f"    {param_names[i]}: [{lo:.4f}, {hi:.4f}] (True: {theta_true[i]:.4f})"
            )
...
fig, axes = plt.subplots(1, len(param_names), sharey=False, figsize=(13, 4))
